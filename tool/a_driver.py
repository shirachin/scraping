...
class A_driver(Driver):
    ...
    def data_pick(self, id) -> Property_api:
        ...
        if data.id.set_param(id):
            ...
            data.building_year = int(
                re.search(r"[\d]+年", buil).group().replace("年", "")
            )
            data.building_month = int(
            ...
            data.floor_plan = floor_plan_translate(
            ...
            data.building_name = self.find_element(
                By.XPATH, data_xpath("建物名・部屋番号")
            ).text
            data.floor_plan_breakdown = self.find_element(
                By.XPATH, data_xpath("間取り内訳")
            ).text
            data.balcony = self.find_element(By.XPATH, data_xpath("バルコニー")).text
            data.building_structure = self.find_element(
            ...
            data.parking_car = self.find_element(
            ...
            data.main_glossy_surface = self.find_element(
                By.XPATH, data_xpath("主要採光面")
            ).text
            data.administrator = self.find_element(
            ...
            data.depreciation_guarantee = self.find_element(
                By.XPATH, data_xpath("保証金償却")
            ).text
            data.rental_guarantee = self.find_element(By.XPATH, data_xpath("賃貸保証")).text
            data.credit_card_payment = self.find_element(
                By.XPATH, data_xpath("クレジットカード決済")
            ).text
            data.insurance_subscription = self.find_element(
            ...
            data.occupancy_requirement = self.find_element(
                By.XPATH, data_xpath("入居条件")
            ).text
            data.equipment_warranty = self.find_element(
